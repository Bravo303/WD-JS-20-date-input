...
function f2() {
  ...
  if (a21 > a22) {
    div.innerHTML = a21; //UPD Исправил, я почему то вместо переменных булевые значение вывел...
  } else {
    div.innerHTML = a22;
  }
}
...
function f3() {
  let inputOne = +document.querySelector('.i-31').value;
  let inputTwo = +document.querySelector('.i-32').value;
...
function f7() {
  let inputOne = +document.querySelector('.i-71').value;
  let inputTwo = +document.querySelector('.i-72').value;
  let div = document.querySelector('.out-7');
  let a = inputOne ** inputTwo;
  div.innerHTML = a;
}
...
function f13() {
  ...
  document.querySelector('.out-13').innerHTML = typeof input; // Потому что "input type number" означает что пользователь может вводить только числа, а не то что числа будут выводится. Если поставить "+" в 190-й строке typeof будет number
}
...
function f14() {
  let inputOne = +document.querySelector('.i-141').value;
  let inputTwo = +document.querySelector('.i-142').value;
...
function f15() {
  let selectOne = +document.querySelector('.s-151').value;
  let selectTwo = +document.querySelector('.s-152').value;
  let selectThree = document.querySelector('.s-153').value;
  let div = document.querySelector('.out-15');
  if (selectThree == '||') {
    // UPD исправил, не правильно задал условие поставил один знак равно, а надо было два, гулпая ошибка
    div.innerHTML = selectOne || selectTwo; // Возвращает первое истенное значение или последнее ложное
  } else {
    div.innerHTML = selectOne && selectTwo; // Возвращает первое ложное значение
  }
}
